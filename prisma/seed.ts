...
async function main() {
  ...
  const clientWorkspace = await prisma.workspace.create({
    data: {
      ...
      members: {
        create: [{ userId: demoUser.id, role: WorkspaceMemberRole.ADMIN }],
      },
    },
  });
  ...
  const techProject = await prisma.project.create({
    data: {
      ...
      description:
        'Weekly tech reviews and tutorials for the YouTube channel. Each video goes through multiple review cycles.',
